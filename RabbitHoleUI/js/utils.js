...
let negativePromptField = document.getElementById('negative_prompt')
let widthField = document.getElementById('width')
let heightField = document.getElementById('height')
let numInferenceStepsField = document.getElementById('num_inference_steps')
let guidanceScaleField = document.getElementById('guidance_scale')
//let useFaceCorrectionField = document.getElementById('use_face_correction')
//let useUpscalingField = document.getElementById('use_upscale')
let upscaleModelField = document.getElementById('use_upscale')
let hypernetworkModelField = document.getElementById('use_hypernetwork_model')
let vaeModelField = document.getElementById('use_vae_model')
let loraAlphaField = document.getElementById('lora_alpha')
let loraModelField = document.getElementById('use_lora_model')
let samplerField = document.getElementById('sampler_name')
let stableDiffusionModelField = document.getElementById('use_stable_diffusion_model')
let promptStrengthField = document.getElementById('prompt_strength')
let gfpganModelField = document.getElementById('use_face_correction')
...
const TASK_MAPPING = {
    ...
    /*use_face_correction: { name: 'Use Face Correction',
        ...
        readUI: () => ((gfpganModelField.value != '') ? gfpganModelField.value : undefined),
        parse: (val) => val
    },
    use_upscale: { name: 'Use Upscaling',
        ...
        readUI: () => ((upscaleModelField.value != '') ? upscaleModelField.value : undefined),
        parse: (val) => val
    },
    ...
    use_vae_model: { name: 'VAE model',
        setUI: (use_vae_model) => {
            ...
            if (use_vae_model !== '') {
                ...
                let part_array = use_vae_model.split("\\")
                use_vae_model = part_array[part_array.length - 1]
    ...
    use_lora_model: { name: 'LoRA model',
        setUI: (use_lora_model) => {
            ...
            if (use_lora_model !== '') {
                ...
                let part_array = use_lora_model.split("\\")
                use_lora_model = part_array[part_array.length - 1]
    ...
    use_face_correction: { name: 'Use Face Correction',
        setUI: (use_face_correction) => {
            const oldVal = gfpganModelField.value
            use_face_correction = (use_face_correction === undefined || use_face_correction === null || use_face_correction === 'None' ? '' : use_face_correction)

            if (use_face_correction !== '') {
                use_face_correction = getModelPath(use_face_correction, ['.pth'])
                let part_array = use_face_correction.split("\\")
                use_face_correction = part_array[part_array.length - 1]
                use_face_correction = use_face_correction !== '' ? use_face_correction : oldVal
            }
            gfpganModelField.value = use_face_correction
        },
        readUI: () => gfpganModelField.value,
        parse: (val) => val
    },
    use_upscale: { name: 'Use Upscaler',
        setUI: (use_upscale) => {
            const oldVal = upscaleModelField.value
            use_upscale = (use_upscale === undefined || use_upscale === null || use_upscale === 'None' ? '' : use_upscale)

            if (use_upscale !== '') {
                use_upscale = getModelPath(use_upscale, ['.pth'])
                let part_array = use_upscale.split("\\")
                use_upscale = part_array[part_array.length - 1]
                use_upscale = use_upscale !== '' ? use_upscale : oldVal
            }
    ...
    use_hypernetwork_model: { name: 'Hypernetwork model',
        setUI: (use_hypernetwork_model) => {
            ...
            if (use_hypernetwork_model !== '') {
                ...
                let part_array = use_hypernetwork_model.split("\\")
                use_hypernetwork_model = part_array[part_array.length - 1]
